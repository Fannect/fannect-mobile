...
$.TokenList = function (input, url_or_data, settings) {
    ...
    this.clear = function() {
        token_list.children(".token").each(function() {
    ...
    function show_dropdown() {
        dropdown
            .css({
                position: "absolute",
                top: $(token_list).offset().top + $(token_list).outerHeight() + 10,
                left: 0,
                zindex: 999
            })
            .show();
    ...
    function populate_dropdown (query, results) {
        if(results && results.length) {
            dropdown.empty();
            var dropdown_ul = $("<div class='listview'>")
                .appendTo(dropdown)
                .mouseover(function (event) {
                    select_dropdown_item($(event.target).closest(".item"));
                })
                .mousedown(function (event) {
                    add_token($(event.target).closest(".item").data("tokeninput"));
                    hidden_input.change();
                    return false;
                })
                .hide();
            ...
            if(settings.noResultsText) {
                dropdown.html("<p class='no-results'>"+settings.noResultsText+"</p>");
                show_dropdown();
            }
        }
    }
